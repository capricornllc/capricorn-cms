...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Partnerpage
      .findOne(_.pick(params, _.keys(Partnerpage.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const relations = _.pick(values, Partnerpage.associations.map(ast => ast.alias));
    const data = _.omit(values, Partnerpage.associations.map(ast => ast.alias));
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Partnerpage.associations.map(a => a.alias));
