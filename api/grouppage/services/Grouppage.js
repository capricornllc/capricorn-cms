...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Grouppage.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Grouppage.updateOne(params, data, { multi: true });
  ...
  /**
   * Promise to search a/an Grouppage.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Grouppage.attributes).reduce((acc, curr) => {
      switch (Grouppage.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
